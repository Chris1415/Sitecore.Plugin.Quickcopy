...
3. Each settled result writes a slice of `PageDerivedState` into the singleton cache at `lib/cache/store.ts`, keyed by the cache key. SDK responses are typed against `@sitecore-marketplace-sdk/xmc` directly — no inferred shapes, no `as never` casts (regression-pinned by `lib/regression-audit.test.ts`). The Live URL is composed eagerly via `new URL(slug, host).toString()` when status is published AND a host string is available; otherwise the slot stores `null` (disabled) or an `Error` (persistent error). Host selection picks the first entry with a non-empty `targetHostname` and falls back to the first non-empty `hostnames[]` value if none qualify — the operator must populate `targetHostname` under XM Cloud → Sites → Hosts for the Live URL to point at the real public site (see `README.md` § "Backend prerequisite").
